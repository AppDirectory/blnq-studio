...
export default () => {
    ...
    const setOpenFin = (value) => {
        dispatch('updateOpenFin', {
            showOpenFin: value
        });
        createEditorSplitter();
    };

    const setOSFrame = (value) => {
    ...
    const openFin = $('pref-OpenFin');
    const openFinPref = localStorage.getItem('pref-openfin');
    if (openFinPref !== null) {
        var pref = localStorage.getItem('pref-openfin');
        if (JSON.parse(pref)) {
            openFin.checked = true;
            setOpenFin(true);
        } else {
            openFin.checked = false;
            setOpenFin(false);
        }
    } else {
        if (typeof fin !== 'undefined') {
            openFin.checked = true; //Set to true when within OpenFin mode
        }
        setOpenFin(openFin.checked);
    }
    openFin.addEventListener('change', function () {
        localStorage.removeItem('editor-layout-sizes');
        localStorage.setItem('pref-openfin', openFin.checked);
        setOpenFin(openFin.checked);
    });

    const osFrame = $('pref-osframe');
    const osFramePref = localStorage.getItem('pref-osframe');
    if (osFramePref !== null) {
        var pref = localStorage.getItem('pref-osframe');
        if (JSON.parse(pref)) {
            osFrame.checked = true;
            setOSFrame(true);
        } else {
            osFrame.checked = false;
            setOSFrame(false);
        }
    } else {
        osFrame.checked = true;
        setOSFrame(osFrame.checked);
    }
    osFrame.addEventListener('change', function () {
        localStorage.setItem('pref-osframe', osFrame.checked);
        setOSFrame(osFrame.checked);
    });
