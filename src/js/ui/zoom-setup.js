...
export default function zoomSetup() {
    ...
    options.forEach((obj) => {
        ...
        obj.options.forEach((obj) => {
            ...
            if (obj.width === 'openfin') {
                option.id = 'zoom-openfin';
                if (store.state.showOpenFin === false) {
    ...
    function workoutzoom() {
        ...
        if (width.indexOf('%') === -1) {
            ...
            if (width === 'openfin') {
                try {
                    var appjson = JSON.parse(store.state.openfin);
                    width = appjson.startup_app.defaultWidth + 'px';
                    height = appjson.startup_app.defaultHeight + 'px';

                    var frame = appjson.startup_app.frame;
                    if (frame) {
                        if (navigator.userAgent.indexOf('Macintosh') > -1) {
                            document
                                .querySelector('.os-overlay')
                                .setAttribute('os', 'mac');
                        }
                ...
                document.querySelector('.os-overlay').removeAttribute('os');
            }
    ...
    window.addEventListener('resize', function () {
        try {
            var openfin = JSON.parse(store.state.openfin);
            //console.log(openfin.startup_app.frame);
        } catch (e) {}
        workoutzoom();
    });
