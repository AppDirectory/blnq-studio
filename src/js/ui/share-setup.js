...
export default () => {
    ...
    <div class="section__body">
        ...
        var links = [
            ...
            {
                title: 'OpenFin View (Requires Desktop Install, Windows Only)',
                url: `${wl.protocol === 'http:' ? 'fin:' : 'fins:'}//${
                    wl.host
                }/f/${store.state.blnqName}.openfin`,
                onlyOpenFin: true
            },
            {
                title:
                    'OpenFin Editor (Requires Desktop Install, Windows Only)',
                url: `${wl.protocol === 'http:' ? 'fin:' : 'fins:'}//${
                    wl.host
                }/openfin/?$$blnqName=${store.state.blnqName}`,
                onlyOpenFin: true
            }
        ];

        const isOpenFin = document
            .querySelector('body')
            .classList.contains('isOpenFin');

        for (var i = 0; i < links.length; i++) {
            if (links[i].onlyOpenFin === false || isOpenFin)
        ...
        var files = [
            ...
            {
                title: 'OpenFin',
                url: `${wl.protocol}//${wl.host}/f/${store.state.blnqName}.openfin`
            }
        ];
