...
const openfinModel = (content, blnqName) =>
    monaco.editor.createModel(content, 'json');
...
const editorSetup = ({
    ...
    const openfin = createEditor(
        editorOptions,
        openfinModel(openfinVal, blnqName),
        document.querySelector('#editorOPENFIN .editor__wrapper')
    );
    openfin.addAction(showConfig);
    openfin.addAction(showPreferences);
    openfin.getModel().updateOptions({ insertSpaces: false, tabSize: 4 });

    editors['openfin'] = openfin;
    ...
    const bindOpenfin = () => {
        ...
        eventDisposers.push(
            openfin.onDidFocusEditorText((e) => {
                var selection = openfin.getSelection();
                updateSelection(
                    selection.startLineNumber,
                    selection.startColumn
                );
        ...
        eventDisposers.push(
            openfin.onDidChangeCursorSelection((e) => {
                var selection = e.selection;
                updateSelection(
                    selection.startLineNumber,
                    selection.startColumn
    ...
    subscribe(
        ...
        debounce(() => {
            ...
            openfin.setValue(store.state.openfin);
            updateDocument(store.state);
            bindEditors();
        }, debounceRate)
    );
    ...
    subscribe('updateModelConfig', (obj) => {
        ...
        openfin.getModel().updateOptions(obj);
    });
    subscribe('updateOpenFin', (obj) => {
        var OFTab = document.querySelector('.btn--tab[data-show="OPENFIN"]');

        if (obj.showOpenFin) {
            OFTab.style.display = 'inline-flex';
        } else {
            OFTab.style.display = 'none';
        }
        try {
            var OFViewOption = document.querySelector('#zoom-openfin');
            if (obj.showOpenFin) {
                OFViewOption.removeAttribute('disabled');
            } else {
                OFViewOption.setAttribute('disabled', true);
            }
        } catch (e) {}
    });
